...
public class MovieIdTests
{
    ...
    public void ProperGuidsShouldCreateMovieId(string id)
    {
        var parsedId = Guid.Parse(id);
        
        var movieId= new MovieId(parsedId);
        
        movieId.Value.ShouldBe(parsedId);
    }
    
    [Test]
    public void EmptyGuidShouldThrowInvalidMovieIdException()
    {
        Should.Throw<InvalidMovieIdException>(() => new MovieId(Guid.Empty));
    }
}